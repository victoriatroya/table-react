...
const server = setupServer(
  rest.get(URL, (req, res, ctx) => {
    return res(ctx.json(tableData));
  }),
  rest.post(URL, (req, res, ctx) => {
    const params = req.url.searchParams.get("idAuthor=1");
    return res(ctx.json([params, { id: 4, text: req.body.text, done: false }]));
  }),
...
test("should delete the pokemon when clicked", async () => {
  ...
  const { getAllByTestId } = render(
    <Table data={tableData} setData={setData} />
  );
  ...
  expect(setData).toHaveBeenCalledWith([
    { attack: 50, defense: 50, id: 2, name: "Pokemon #2" },
  ]);
